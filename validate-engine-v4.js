...
r=E.evaluate({...base,delayed:{enabled:true,originalMin:420,actualMin:659,undisturbed:false}});eq(r.maxFDPHours,14,'CAT delay 3:59 uses original band');eq(r.fdpStartMin,659,'CAT delay <4 starts actual');
r=E.evaluate({...base,delayed:{enabled:true,originalMin:420,actualMin:660,undisturbed:false}});eq(r.maxFDPHours,14,'CAT delay 4h limiting bands');eq(r.fdpStartMin,660,'CAT delay 4 starts original+4');
...
for(const rule of ['NCC','CAT']){const L=E.TABLES[rule].cumulative;r=E.cumulative(rule,{d7:L[0],d14:L[1],d28:L[2],d12:L[3],f28:100,f12:900,consecutiveDutyDays:7,daysOff28:7,twoConsecutiveDaysOffOK:true,daysOffYear:rule==='CAT'?96:undefined});ok(r.legal,`${rule} equal limits legal`);r=E.cumulative(rule,{d7:L[0]+.01,d14:0,d28:0,d12:0,f28:0,f12:0,consecutiveDutyDays:0,daysOff28:7,twoConsecutiveDaysOffOK:true,daysOffYear:rule==='CAT'?96:undefined});ok(!r.legal,`${rule} d7 over illegal`)}
